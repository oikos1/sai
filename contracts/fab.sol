...
contract DaiFab is DSAuth {
    ...
    function setVoxTub(SaiVox _vox, SaiTub _tub)  public auth {
        //require(step == 1);
        vox = _vox;
        tub = _tub;
        step += 1;        
    ...
    function setTapTop(SaiTap _tap, SaiTop _top) public auth {
       //require(step == 2);        
        tap = _tap;
        top = _top;
        step += 1;        
    ...
    function configParams() public auth {
        ...
        tub.mold("cap", 0); 
        tub.mold("mat", ray(1500000000000000000));
        tub.mold("axe", ray(1130000000000000000));
        tub.mold("fee",     1000000000158153903837946257);  // 0.5% / year
        tub.mold("tax", ray(1000000000000000000));

        tub.mold("gap",     1000000000000000000);
        tap.mold("gap",     970000000000000000);
    ...
    function configAuth(DSAuthority authority, SaiMom _mom, DSGuard _dad) public auth {
        //require(step == 5);
        require(address(authority) != 0x0);

        mom = _mom;  //momFab.newMom(tub, tap, vox);
        dad = _dad; //dadFab.newDad();
